...
def clean_carolyn_hax_text(text: str) -> str:
    ...
    for line in lines:
        ...
        line = re.sub(r'Post has \d+ replies? and \d+ Likes?', '', line, flags=re.IGNORECASE)
        line = re.sub(r'Post has undefined replies? and \d+ Likes?', '', line, flags=re.IGNORECASE)
        ...
        if line.strip() in ['and', 'or', 'the', 'a', 'an', 'to', 'of', 'in', 'is', 'it']:
            continue
        ...
        if 'Carolyn HaxAdvice Columnist' in line and not line.strip().startswith('QUESTION') and not line.strip().startswith('ANSWER'):
        ...
        if line.strip() == 'Guest' or line.strip().startswith('Guest ') and ':' not in line:
            continue
...
def main():
    ...
    parser.add_argument(
        '-o', '--output',
        help='Output file (default: input_file with _cleaned suffix)'
    )
    parser.add_argument(
        '-v', '--verbose',
        action='store_true',
    ...
    if args.verbose:
        ...
        print(f"Reduction: {len(original_text) - len(cleaned_text):,} characters removed")
    ...
    print(f"   Original: {len(original_text):,} chars → Cleaned: {len(cleaned_text):,} chars")
