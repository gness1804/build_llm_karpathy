...
def main():
    parser = argparse.ArgumentParser(
        description='Prepare OpenAI fine-tuning dataset from Q&A markdown file.'
    )
    parser.add_argument(
        'source',
        type=str,
    ...
    if args.output:
        ...
        # Default: sources/v2/openai_finetune_YYYYMMDD_HHMMSS.jsonl
        dest_path = Path(f"sources/v2/openai_finetune_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jsonl")
    ...
    try:
        ...
        print(f"Successfully created dataset: {dest_path}")
