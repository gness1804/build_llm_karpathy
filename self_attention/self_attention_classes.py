...
class Head(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        B: int = x.shape[0]  # Extract batch size
        ...
        out: torch.Tensor = weight @ v  # (B, T, T) @ (B, T, head_size) -> (B, T, head_size)
        return out
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads: int, head_size: int, n_embd: int, block_size: int, dropout: float):
        ...
        """
        super().__init__()
        self.heads: nn.ModuleList = nn.ModuleList([Head(n_embd, head_size, block_size, dropout) for _ in range(num_heads)])
